...
async function runBackgroundUploads(
  caseNumber: string,
  all: Array<{ uri: string; mediaType: "image" | "video"; isPrimary: boolean }>,
  ...
  const uploadOne = async (item: {
    ...
    isPrimary: boolean;
  }): Promise<ConfirmImageItem | null> => {
    const { uri, mediaType, isPrimary } = item;
    const isVideo = mediaType === "video";
    const ext = isVideo
      ? uri.split(".").pop()?.toLowerCase() === "mov"
        ? "mov"
        : "mp4"
      : "jpg";
    ...
    try {
      ...
      const current = useIntakeStore.getState().uploadProgress;
      if (current?.caseNumber === caseNumber) {
        useIntakeStore.getState().setUploadProgress({ ...current, done });
      }
    }
  };
  ...
  if (phase === "done") {
    setTimeout(() => {
      const cur = useIntakeStore.getState().uploadProgress;
      if (cur?.caseNumber === caseNumber)
        useIntakeStore.getState().setUploadProgress(null);
    }, 4000);
  }
}
...
export default function OcrPreviewScreen() {
  ...
  const { vehicleNumber, setVehicleNumber, formData, setCreatedCaseNumber } =
    useIntakeStore();

  const [scanning, setScanning] = useState(true);
  const [vn, setVn] = useState(vehicleNumber || "");
  const [ocrDetected, setOcrDetected] = useState(false);
  const [ocrSource, setOcrSource] = useState<"paddle" | "textract" | null>(
    null,
  );
  ...
  const { setUploadProgress } = useIntakeStore();
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      // Snapshot URIs while still in component context — temp files still live
      const all = [
        ...(formData.primaryImage
          ? [
              {
                uri: formData.primaryImage,
                mediaType: "image" as const,
                isPrimary: true,
              },
            ]
          : []),
        ...formData.additionalImages.map((m) => ({
          uri: m.uri,
          mediaType: m.type,
          isPrimary: false,
        })),
    ...
    onSuccess: ({ created, all }) => {
      ...
      if (all.length > 0) {
        ...
        void runBackgroundUploads(created.caseNumber, all, queryClient);
      }
      ...
      <View style={[styles.content, { paddingBottom: insets.bottom + 24 }]}>
          ...
          >
            <Animated.View
              style={[
                styles.resultHeader,
                {
                  ...
                  transform: [
                    {
                      translateY: resultAnims[0].interpolate({
                        inputRange: [0, 1],
                        outputRange: [10, 0],
                      }),
                    },
            ...
            <Animated.View
              style={{
                ...
                transform: [
                  {
                    translateY: resultAnims[1].interpolate({
                      inputRange: [0, 1],
                      outputRange: [10, 0],
                    }),
                  },
            ...
            <Animated.View
              style={{
                ...
                transform: [
                  {
                    translateY: resultAnims[2].interpolate({
                      inputRange: [0, 1],
                      outputRange: [8, 0],
                    }),
                  },
            ...
            >
              <Text style={styles.editHint}>
                Tap to edit if OCR result is incorrect
              </Text>
            </Animated.View>
            ...
            <Animated.View
              style={[
                styles.metaRow,
                {
                  ...
                  transform: [
                    {
                      translateY: resultAnims[3].interpolate({
                        inputRange: [0, 1],
                        outputRange: [8, 0],
                      }),
                    },
        ...
        <View style={styles.footer}>
          <Pressable
            ...
            style={[
              styles.createBtn,
              (scanning || !vn.trim() || createMutation.isPending) &&
